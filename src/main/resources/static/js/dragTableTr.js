...
if ("setDragImage" in window.DataTransfer.prototype && document.body.animate) {
    ...
    HTMLElement.prototype.initDraggable = function () {
        ...
        this.addEventListener('dragstart', function (ev) {
            ...
            for (var i = 0; i < fakeObj.children.length; i++) {
              fakeObj.children[i].style.display = 'inline-block'
              fakeObj.children[i].style.padding ='5px 0 0';
              if(i<4) {
                if(i === 1) fakeObj.children[i].style.flex = '0.3'
                 else fakeObj.children[i].style.flex = '1'
              }
            }

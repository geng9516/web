...
import jp.smartcompany.job.modules.core.util.appcontrol.AbstractInfo;
...
/**
 * 検索対象範囲設定情報取得クラス --> jp.co.scientia.framework.util.logic.impl.PsBuildTargetSqlLogicImpl
 * @author Xiao Wenpeng
 */
@Component
...
public class PsBuildTargetSql {
    ...
    /** 起動権限情報取得Logic */
    private final AppAuthUtil appAuthInfoLogic;
    /** アプリケーション別検索範囲取得処理クラス */
    private final AppSearchRangeUtil appSearchRangeInfoLogic;
    private final TableCombinationTypeCache tableCombinationTypeCache;

    // ログイン情報クラス取得
    private PsSession gPsSession;
    /** セキュリティ基準日 */
    private String gsPsSecurityDate;	// YYYY/MM/DD
    ...
    private void create(String psSiteId, String psMode) {
        ...
        this.appSearchRangeInfoLogic.setPsSite(sSiteId);
        this.appSearchRangeInfoLogic.create(this.getApplicationUrl(), psMode);
    }
    /**
     * 検索対象範囲情報を構築する。
     * @param psSiteId サイトId
    ...
    private void createDomain(String psSiteId, String psDomainCode, String psMode) {
        ...
        this.appSearchRangeInfoLogic.setPsSite(sSiteId);
        this.appSearchRangeInfoLogic.create(this.getApplicationUrl(), psDomainCode, psMode);
    }
    /**
     * 検索対象範囲情報を構築する。
     * @param psSiteId サイトId
    ...
    private void create(String psSiteId, String psAppId, String psMode) {
        ...
        this.appSearchRangeInfoLogic.setPsSite(sSiteId);
        this.appSearchRangeInfoLogic.setPsApp(psAppId);
        this.appSearchRangeInfoLogic.create(this.getApplicationUrl(), psMode);
    }
    /**
     * 検索対象範囲情報を構築する。
    ...
    private List < String > getTargetSqlFroms() {
        return this.appSearchRangeInfoLogic.getFrom();
    }
    /**
     * 検索対象範囲設定SQL組み立て処理
     * (Where<条件式用>句)
    ...
    private String getTargetSqlWhere() {
        return this.appSearchRangeInfoLogic.getCondWhere();
    }
    /**
     * 検索対象範囲設定SQL組み立て処理
     * (Where<結合式用>句)
    ...
    private List < String > getTargetSqlWheres() {
        return this.appSearchRangeInfoLogic.getCombWhere();
    }
    /**
     * 検索対象範囲設定SQL組み立て処理
     * (設定済みフラグ)
    ...
    private boolean isSetupFlg() {
        return this.appSearchRangeInfoLogic.isCondSetupFlg();
    }
    /**
    ...
    protected int getCondRetired() {
        return this.appSearchRangeInfoLogic.getCondRetired();
    }
...
// サイトID、アプリケーションID(V3互換)を取得
        ...
        if (inf != null) {
            ...
            if (this.getDomain().equals(PsConst.DOMAIN_EMPLOYEE)) {
                this.appSearchRangeInfoLogic.setPsTargetUser(
                        (String) this.request.getParameter(PsConst.PARAM_KEY_USERID));
            } else if (this.getDomain().equals(PsConst.DOMAIN_ORGANIZATION)) {
                this.appSearchRangeInfoLogic.setPsSelectedComp(
                        (String) this.request.getParameter(PsConst.PARAM_KEY_COMP));
                this.appSearchRangeInfoLogic.setPsSelectedDept(
                        (String) this.request.getParameter(PsConst.PARAM_KEY_DEPT));
            }
        }
        // createしてなかったら処理続行
    ...
    protected void createSearchRange(String psDomainCode, String psMode) {
        ...
        // サイトID、アプリケーションID(V3互換)を取得
        String sSiteId = this.request.getParameter(PsConst.PARAM_KEY_SITEID);
        String sAppId = this.request.getParameter(PsConst.PARAM_KEY_APPID);
        ...
        if (sAppId == null) {
            // V4
            this.createDomain(sSiteId, psDomainCode, psMode);
        } else {
            // V3互換
            this.createDomain(sSiteId, sAppId, psDomainCode, psMode);
        }
        ...
        if (this.getDomain().equals(PsConst.DOMAIN_EMPLOYEE)) {
            ...
            this.appSearchRangeInfoLogic.setPsTargetUser(this.request.getParameter(PsConst.PARAM_KEY_USERID));

        } else if (this.getDomain().equals(PsConst.DOMAIN_ORGANIZATION)) {

            // 組織ドメイン

            this.appSearchRangeInfoLogic.setPsSelectedComp(request.getParameter(PsConst.PARAM_KEY_COMP));

            this.appSearchRangeInfoLogic.setPsSelectedDept(request.getParameter(PsConst.PARAM_KEY_DEPT));

        } else {
            // 法人ドメイン
            appSearchRangeInfoLogic.setPsSelectedComp(this.request.getParameter(PsConst.PARAM_KEY_COMP));
        }
    ...
    protected String convertWhereInfoToSql(List < String > plWhere, String psWheres) {
        ...
        if (plWhere != null && !plWhere.isEmpty()) {
            ...
            while (i.hasNext()) {
                ...
                if (sQueryWhere != null && !sQueryWhere.equals("")) {
                    sb.append(AND + sQueryWhere);
                }
            }
        }
        // 検索対象範囲情報：Where句(条件式用)
    ...
    public String getExistsQueryHdId(String psUserColName, String psIdColName) {
        ...
        for (String sFrom : this.gFromListMap.get(this.getDomain())) {
            ...
            // 対象テーブル取得
            // 使用するテーブルの情報を取得
            TableCombinationType tableFromType
                    = tableCombinationTypeCache.getTableCombinationType(sFrom);
        ...
        for (String sConditionFrom : this.gFromListMap.get(this.getDomain())) {
            ...
            TableCombinationType tableCombinationType
                    = tableCombinationTypeCache.getTableCombinationType(sConditionFrom);
            if (!StrUtil.equalsIgnoreCase(tableCombinationType.getTableName(),"HIST_DESIGNATION")) {
                continue;
            }
            // ユーザIDを保有するテーブルに結合条件を記述
            String sIdColumnName = tableCombinationType.getIdColumnName();
            if (StrUtil.isNotBlank(sIdColumnName)) {
                // ユーザID
                if (nIndex != 0) {
                    sb.append(AND);
        ...
        if (gbSetFlg) {
            ...
            for (String sConditionFrom : this.gFromListMap.get(this.getDomain())) {
                ...
                TableCombinationType tableCombinationType
                        = tableCombinationTypeCache.getTableCombinationType(sConditionFrom);
                // 法人コードと組織コードの両方を保有するテーブルに結合条件を記述
                if (StrUtil.isNotBlank(tableCombinationType.getCompanyIdColumnName()) &&
                        StrUtil.isNotBlank(tableCombinationType.getEmployeeIdColumnName())) {
                    // 法人コード
                    if (nIndex != 0) {
                    ...
                    sbExistsQuery.append(tableCombinationType.getCompanyIdColumnName());
                    ...
                    sbExistsQuery.append(EXSISTB + ".");
                    sbExistsQuery.append("EX_");
                    sbExistsQuery.append(tableCombinationType.getEmployeeIdColumnName());
                    sbExistsQuery.append(" = ");
    ...
    public String getExistsQueryOrganisation(String psCompanyColName, String psOrganisationColName) {
        ...
        if (gbSetFlg) {
            ...
            for (String sConditionFrom : gFromListMap.get(this.getDomain())) {
                ...
                // 法人コードと組織コードの両方を保有するテーブルに結合条件を記述
                sCompanyIdColumnName = tableCombinationType.getCompanyIdColumnName();
                sSectionIdColumnName = tableCombinationType.getSectionIdColumnName();
                // カラムを持っていないテーブル用判定
                if (StrUtil.isNotBlank(sCompanyIdColumnName) && StrUtil.isNotBlank(sSectionIdColumnName)) {
    ...
    public String getExistsQueryCompany(String psCompanyColName) {
        ...
        // パラメータエラーチェック
        if (psCompanyColName == null || psCompanyColName.equals("")) {
            // カラム指定されていないので例外
            throw new GlobalException(
                    "psCompanyColName"+ERR_NOT_NULL);
        }
        ...
        if (gbSetFlg) {
            ...
            sExistsQuery = replaceColumeNameString(this.gWhereSqlMap.get(this.getDomain()), psCompanyColName, "", "");
        }
        return sExistsQuery;
    }
    ...
    private String getExistsQueryCommon(String psIndividualCondition) {
        ...
        for (String sConditionFrom : this.gFromListMap.get(this.getDomain())) {
            ...
            String sCompIdColumnName = tableCombinationType.getCompanyIdColumnName();
            String sSectionIdColumnName = tableCombinationType.getSectionIdColumnName();
            String sEmployeeColumnName = tableCombinationType.getEmployeeIdColumnName();


            if (StrUtil.equals(domain,"01")) {
                // 別名付与(従業員）
                boolean isColValid = StrUtil.isNotBlank(tableCombinationType.getCompanyIdColumnName()) &&
                        StrUtil.isNotBlank(tableCombinationType.getEmployeeIdColumnName());
                if (isColValid) {
                    // 法人コード
                    //sb.append(PsBuildTargetSqlLogicImpl.AND);
                    ...
                    sb.append(PT_ID).append(nIndex).append(".");
                    sb.append(tableCombinationType.getCompanyIdColumnName());
                    sb.append(AS);
                    sb.append(" EX_");
                    ...
                    //sb.append(PsBuildTargetSqlLogicImpl.AND);
                    sb.append(PT_ID).append(nIndex).append(".");
                    sb.append(tableCombinationType.getEmployeeIdColumnName());
                    sb.append(AS);
                    sb.append(" EX_");
    ...
    public String getDesignationQuery() {
        ...
        for (String sFrom : this.gFromListMap.get(this.getDomain())) {
            // 対象テーブル取得
            // 使用するテーブルの情報を取得
            TableCombinationType tableFromType = tableCombinationTypeCache.getTableCombinationType(sFrom);
            // 異動歴の場合
            if (StrUtil.equalsIgnoreCase(tableFromType.getTableName(),"HIST_DESIGNATION")) {
                // 異動歴のレコード
                sb.append(PT_ID).append(nIndex).append(".* ");

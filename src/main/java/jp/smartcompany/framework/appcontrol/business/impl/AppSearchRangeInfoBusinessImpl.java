...
import jp.smartcompany.framework.util.PsTableCombinationType;
...
/**
 * アプリケーション別検索範囲取得Logic実装クラス
 * @author Xiao Wenpeng
 */
@Service
@RequiredArgsConstructor(onConstructor = @__(@Autowired))
public class AppSearchRangeInfoBusinessImpl implements AppSearchRangeInfoBusiness {
    /** ログイン情報クラス */
...
//            sSystemCode = this.gAppAuthInfoLogic.getSystemCode(sSiteId, sApplicationId);
    ...
    private String createPermissionQuery(String sPermissionID, String sCustomerID, String sDomainId){
        ...
        // 組織・役職指定の場合
        searchRangeInfoDTOList = scCacheUtil.getDataSectionPost(sPermissionID);
        ...
        if (sPermissionID.equals(CODE_0001)){
            ...
            // 条件式指定の場合
            searchRangeInfoDTOList = scCacheUtil.getDataPermissionDefs(sPermissionID);
    ...
    private String getSetDefault(String psTargetUser, String psTargetCompany, String psTargetSection, String psDomain) {
        ...
        if (psDomain.equalsIgnoreCase(PsConst.DOMAIN_EMPLOYEE)) {
            // 条件式：従業員ドメイン"01"の場合
            sWheres.append(this.escDBString(gPsSession.getLoginUser())).append(PT_EQUAL)
                    .append(this.escDBString(psTargetUser));
            sWheres.append(PT_AND + "#HD_USER#" + PT_EQUAL).append(this.escDBString(psTargetUser));
            ...
            for (Designation value : designation) {
                // 法人コード比較
                sWheres.append(PT_OPEN_PAR + "#MO_COMP#" + PT_EQUAL)
                        .append(this.escDBString(value.getCompanyCode()));
                // 組織コード比較
                sWheres.append(PT_AND + "#MO_DEPT#" + PT_EQUAL)
                        .append(this.escDBString(value.getSection())).append(PT_CLOSE_PAR);
            ...
            for (Designation value : designation) {
                // 法人コード比較
                sWheres.append(PT_OPEN_PAR + "#MC_COMP#" + PT_EQUAL)
                        .append(this.escDBString(value.getCompanyCode()))
                        .append(PT_CLOSE_PAR);
                sWheres.append(PT_OR);
            }
    ...
    private String createCondWhere(List<String> conditionList, String sConnective) {
        ...
        if (!conditionList.isEmpty()) {
            ...
            for (Iterator<String> conditionIterator = conditionList.iterator(); conditionIterator.hasNext();) {
                ...
                if (StrUtil.isNotBlank(sCondition)) {
                    sbQuery.append(PT_OPEN_PAR).append(sCondition).append(PT_CLOSE_PAR);
    ...
    private String setDataPermissionDefs(List<SearchRangeInfoDTO> searchRangeInfoDTOList, String psCustomerId, String psDomain) {
        ...
        for (int i = 0; i < searchRangeInfoDTOList.size(); i++) {
            ...
            TableCombinationTypeDTO tableCombinationTypeDTO
                    = scCacheUtil.getTableCombinationType(sMdpdCtableid);
            if (tableCombinationTypeDTO == null) {
                break;
            }
            ...
            if (searchRangeInfoDTO.getMdpdCmyflag().equals(MY_FLG2) || searchRangeInfoDTO.getMdpdCmyflag().equals(MY_FLG3)) {
                ...
                sMasterRecodeQuery = this.createNormalSQL(tableCombinationTypeDTO, searchRangeInfoDTO, psDomain);
            }
    ...
    private String createMasterSpecialMeaningSQL(SearchRangeInfoDTO searchRangeInfoDTO, String psDomain) {
        ...
        String sMdpdCtableid = searchRangeInfoDTO.getMdpdCtableid();
        String sMdCmastertblname = searchRangeInfoDTO.getMdCmastertblname();
        TableCombinationTypeDTO tableCombinationTypeDTO = scCacheUtil.getTableCombinationType(sMdpdCtableid);
        ...
        for (Iterator<Designation> designationIte = designationList.iterator(); designationIte.hasNext();) {
            ...
            if (!this.isEmpty(sMdCmastertblname) && sMdCmastertblname.equals(QCOMPANY)) {
                ...
                sbQuery.append(tableCombinationTypeDTO.getCompanyIdColumnName());
                sbQuery.append(PT_SPACE);
                ...
                if(searchRangeInfoDTO.getMdpdCoperator().equals(PT_ISNULL) ||
                    ...
                    sbQuery.append(tableCombinationTypeDTO.getSectionIdColumnName());
                    sbQuery.append(PT_SPACE);
                    sbQuery.append(searchRangeInfoDTO.getMdpdCoperator());
                }else{
                ...
                if(searchRangeInfoDTO.getMdpdCoperator().equals(PT_ISNULL) ||
                        searchRangeInfoDTO.getMdpdCoperator().equals(PT_ISNOTNULL)){
                    sbQuery.append(tableCombinationTypeDTO.getPostIdColumnName());
                    sbQuery.append(PT_SPACE);
                    sbQuery.append(searchRangeInfoDTO.getMdpdCoperator());
                }else{
                    // 法人：MO_CCOMPANYID_CK_FK = '' AND
                    ...
                    if (searchRangeInfoDTO.getMdpdCoperator().equals("=")){
                        ...
                        sbQuery.append(tableCombinationTypeDTO.getPostIdColumnName() + PT_SPACE);
    ...
    private List<Designation> createMasterSpecialMeaningData(SearchRangeInfoDTO searchRangeInfoDTO) {
        ...
        List<Designation> designationList = CollUtil.newLinkedList();
        ...
        if (searchRangeInfoDTO.getMdpdCmyflag().equals(MY_FLG0)) {
            ...
            if (!this.isEmpty(sMdCmastertblname) && sMdCmastertblname.equals(QCOMPANY)) {
                ...
                for (Designation designation : this.gPsSession.getLoginDesignation()) {
                    // 初期化
        ...
        if (!this.isEmpty(sMdCmastertblname) && sMdCmastertblname.equals(QCOMPANY)) {
            ...
            /** 組織 */
            List<Designation> newDesignationList = CollUtil.newLinkedList();

            // 対象リスト(指定値および異動歴)毎のLOOP
            for (Designation designation : designationList) {

                List<String> tergetSectionList = CollUtil.newLinkedList();
                if (StrUtil.equals(searchRangeInfoDTO.getMdpdCoperator(), ">") || searchRangeInfoDTO.getMdpdCoperator().equals(">=")) {
                    // 上位情報
                    tergetSectionList = sysInfoBusiness.getUpperSectionListDeptForSQL(designation.getCustomerCode(),
                            designation.getCompanyCode(), designation.getSection(), getPsSecurityDate4DateType());
                } else if (StrUtil.equals(searchRangeInfoDTO.getMdpdCoperator(), "<") || searchRangeInfoDTO.getMdpdCoperator().equals("<=")) {
                    // 下位情報
                ...
                if (tergetSectionList != null && !tergetSectionList.isEmpty()) {
                    for (String sSection : tergetSectionList) {
                        // セクションＩＤ待避
    ...
    String createBaseSectionSQL(String psSystemId, String psDomainId, String psGroupId) {
        ...
        sResult = checkWhereList(sResult);
    ...
    private String createSectionPostMappingSQL(SearchRangeInfoDTO searchRangeInfoDTO, String psTableID) {
        ...
        // テーブル情報を取得
        TableCombinationTypeDTO tableCombinationTypeDTO
                = scCacheUtil.getTableCombinationType(psTableID);
        ...
        String sPostIdColumnName	= tableCombinationTypeDTO.getPostIdColumnName();
        String sEmployeeIdColumnName= tableCombinationTypeDTO.getEmployeeIdColumnName();
     ...
     * 一般カラムの比較条件SQLを生成します
     *
     * @param   tableCombinationTypeDTO        データディクショナリ(カラム)情報
     * @param   paSearchRangeInfoDTO   検索データ取得結果
     * @param   psDomainId          ドメインコード
     * @return  String              一般のカラムを検索するSQL
     */
    private String createNormalSQL(TableCombinationTypeDTO tableCombinationTypeDTO, SearchRangeInfoDTO paSearchRangeInfoDTO, String psDomainId) {
        ...
        if (sMyFlag.equals(MY_FLG0)) {
            ...
            // カラムID：顧客コード
            String sCustomerIdColumnName = tableCombinationTypeDTO.getCustomerIdColumnName();
            ...
            // カラムID：開始日
            String sStartDateColumnName = tableCombinationTypeDTO.getStartDateColumnName();
            ...
            // カラムID：終了日
            String sEndDateColumnName = tableCombinationTypeDTO.getEndDateColumnName();
    ...
    protected String sectionsSeparateWhere(String psSQLColumn, String psSections, boolean pbEqual) {
        ...
        HashMap<String, String> mapSeparatePart =MapUtil.newHashMap();
